  ...
  describe '#cache' do
    ...
    context 'data not cached' do
      it 'should yield control if there is no valid cache' do
        expect { |b| JSONCache::FileCache.cache(@key, @options, &b) }.to(
          yield_control.once)
      end
      it 'should yield without args' do
        expect { |b| JSONCache::FileCache.cache(@key, @options, &b) }.not_to(
          yield_with_args)
      end
      it 'should create a cache' do
        expect(File.exist?(@cache)).to be false
        JSONCache::FileCache.cache(@key, @options) { @sample_data }
    ...
    context 'data cached' do
      before :each do
        @cached_data = JSONCache::FileCache.cache(@key, @options) { @sample_data }
      end
      ...
      it 'should return the old data if the cache is valid' do
        ...
        valid_cache_data = JSONCache::FileCache.cache(@key, @options) { new_data }
        expect(valid_cache_data).to eq @cached_data
      end
      it 'should return the new data if the cache is invalid' do
        new_data = { 'seeya' => 'later' }
        invalid_cache_data = JSONCache::FileCache.cache(
          @key,
